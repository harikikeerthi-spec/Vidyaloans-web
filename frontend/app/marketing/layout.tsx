...
const SECTION_TITLES: Record<string, string> = {
    ...
    "/marketing/dashboard": "Growth Overview",
    "/marketing/campaigns": "Campaigns Hub",
    "/marketing/leads": "Leads & Audiences",
    "/marketing/referrals": "Referral Engine",
    "/marketing/content": "Content & SEO",
    "/marketing/channels": "Channels & Attribution",
};
...
export default function MarketingLayout({ children }: { children: React.ReactNode }) {
    ...
    const navLinks = useMemo(() => [
        { href: "/marketing/dashboard", icon: "rocket_launch", label: "Growth Overview" },
        { href: "/marketing/campaigns", icon: "campaign", label: "Campaigns Hub", badge: "AI Ready" },
        { href: "/marketing/leads", icon: "groups", label: "Leads & Audiences" },
        { href: "/marketing/referrals", icon: "share", label: "Referral Engine", badge: "Top 10" },
        { href: "/marketing/content", icon: "article", label: "Content & SEO" },
        { href: "/marketing/channels", icon: "hub", label: "Channels & Attribution" },
    ], []);

    const isCurrentActive = (path: string) => {
        if (path === "/marketing/dashboard" && (pathname === "/marketing" || pathname === "/marketing/dashboard")) return true;
        return pathname.startsWith(path);
    };

    const currentTitle = SECTION_TITLES[pathname] || "Marketing Growth Hub";
    ...
    return (
        <div
            className="marketing-dashboard-shell h-screen overflow-hidden flex bg-[#f8fafc] text-slate-900 text-sm font-sans"
        ...
        >
            ...
            >
                ...
                <div className="h-14 px-4 flex items-center border-b border-slate-800 flex-shrink-0 gap-2.5">
                    ...
                    >
                        {siteSettings?.siteName || "VidyaLoans"}<span className="text-indigo-400"> Marketing</span>
                    </span>
                </div>
                ...
                <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto custom-scrollbar">
                    ...
                    >
                        <div className="w-6 h-6 flex items-center justify-center flex-shrink-0">
                            <span className="material-symbols-outlined text-[18px] text-emerald-400">insights</span>
                        </div>
                        <span
                            className={`flex-1 transition-all duration-200 whitespace-nowrap truncate ${
                                sidebarOpen ? "opacity-100" : "opacity-0 w-0 group-hover/sidebar:opacity-100 group-hover/sidebar:w-auto"
                ...
                <div className="p-4 border-t border-slate-800 bg-slate-900/50 flex-shrink-0">
                    <div className="flex items-center gap-3 mb-3 p-1">
                        <img
                            src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.email || "marketing"}`}
                            alt="Avatar"
                        ...
                        >
                            <p className="text-[12px] font-medium text-slate-200 truncate">
                                {user?.firstName ? `${user.firstName} ${user.lastName || ""}` : (user?.email?.split("@")[0] || "Growth Officer")}
                            </p>
                            <p className="text-[10px] text-purple-400 font-mono truncate">Acquisition & Growth</p>
                        </div>
                    </div>
                    <button
            ...
            >
                ...
                <header className="h-[60px] bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-6 flex items-center justify-between sticky top-0 z-40 flex-shrink-0 shadow-sm">
                    ...
                    {/* Right: Growth Status Badge, Clock, Date Selector, Actions */}
                    <div className="flex items-center gap-3">
                        <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-50 border border-indigo-200 text-[#4F46E5] text-[11px] font-semibold">
