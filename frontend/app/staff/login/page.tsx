...
function StaffLoginContent() {
    ...
    const verifyOtp = async () => {
        ...
        try {
            ...
            const redirectTo = searchParams.get("redirect");
            router.push(redirectTo ? decodeURIComponent(redirectTo) : "/staff/dashboard");
        } catch (e: unknown) {
            ...
            <div className="relative z-10 w-full max-w-md">
                ...
                <div className="text-center mb-8">
                    <Link href="/" className="inline-flex items-center gap-2 mb-6">
                        <div className="w-12 h-12 rounded-2xl bg-[#1d4ed8]/10 flex items-center justify-center border border-[#1d4ed8]/20">
                            <span className="material-symbols-outlined text-[#1d4ed8] text-3xl">support_agent</span>
                        </div>
                        <span className="font-bold text-3xl font-display text-gray-900">Staff Portal</span>
                    </Link>
                    <h1 className="text-3xl font-bold text-gray-900 font-display mb-2">
                        {step === "email" ? "Staff Access" : "Verify Identity"}
                    </h1>
                    <p className="text-gray-600 text-sm">
                        {step === "email"
                            ? "Please enter your staff email to continue"
                            : `A secure code has been sent to ${email}`}
                    </p>
                </div>

                {/* Login Card */}
                <div className="bg-white/80 backdrop-blur-2xl border border-white/50 rounded-[2.5rem] p-10 shadow-2xl shadow-blue-500/10 border-t-4 border-t-[#1d4ed8]">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3 ml-1">Staff Email</label>
                            <div className="relative">
                                ...
                                {step === "otp" && (
                                    <button
                                        ...
                                        className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-[#1d4ed8] hover:underline"
                                    >
                                        Edit
                                    </button>
                                )}
                            ...
                            <div className="animate-fade-in">
                                ...
                                <div className="flex gap-2 justify-between" onPaste={handleOtpPaste}>
                                    {otp.map((digit, i) => (
                                        <input
                                            ...
                                            className="w-12 h-16 text-center text-2xl font-bold bg-gray-50 border border-gray-100 rounded-2xl text-gray-900 focus:outline-none focus:ring-4 focus:ring-[#1d4ed8]/5 focus:bg-white transition-all"
                                        />
                                    ))}
                                </div>
                                <div className="flex justify-between items-center mt-6 px-1">
                                    ...
                                    {resendDisabled ? (
                                        ...
                                        <button type="button" onClick={sendOtp} className="text-xs text-[#1d4ed8] hover:underline font-black uppercase tracking-wider">
                                            Resend Now
                                        </button>
                                    )}
                                </div>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-5 bg-[#1d4ed8] text-white font-black uppercase tracking-widest text-xs rounded-2xl hover:bg-[#1e40af] active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-3 shadow-xl shadow-blue-500/20"
                        >
                            {loading ? (
